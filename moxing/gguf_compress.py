...
class MultiCompressor:
    ...
    def compress(
        self, input_path: Path, output_path: Optional[Path] = None, keep_original: bool = True
    ) -> CompressedGGUFInfo:
    ...
    def decompress(
        self, input_path: Path, output_path: Optional[Path] = None, keep_compressed: bool = True
    ) -> Tuple[Path, float]:
        if not input_path.exists():
            raise FileNotFoundError(f"File not found: {input_path}")
...
class TransparentDecompressor:
    ...
    def get_decompressed_path(self, compressed_path: Path) -> Path:
        ...
        cache_name = stem if stem.endswith(".gguf") else f"{stem}.gguf"
        ...
        with self._lock:
            ...
            result_path, _ = self._compressor.decompress(
                compressed_path, cache_path, keep_compressed=True
            )
            return result_path
...
def benchmark_compression(
    input_path: Path, algorithms: Optional[List[str]] = None
...
def print_benchmark_results(results: List[CompressedGGUFInfo]):
    ...
    if len(results) > 1:
        ...
        console.print(
            f"\n[green]Best compression: {best_ratio.algorithm} "
            f"({best_ratio.savings_percent:.1f}% saved)[/green]"
        )
        console.print(
            f"[green]Fastest: {best_speed.algorithm} ({best_speed.compression_time:.1f}s)[/green]"
        )
