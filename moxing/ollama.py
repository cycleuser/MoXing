...
def _check_port_open(host: str, port: int, timeout: float = 1.0) -> bool:
    ...
    try:
        ...
        logger.debug("Ollama API request failed: %s", e, exc_info=True)
        return False
...
class OllamaModel:
    ...
    def context_str(self) -> str:
        if not self.context_length:
            return "-"
        if self.context_length >= 1024 * 100 or self.context_length >= 1024:
...
def read_gguf_context_length(gguf_path: str) -> Tuple[Optional[int], Optional[str]]:
    ...
    try:
        with open(gguf_path, "rb") as f:
            ...
            for _i in range(min(metadata_kv_count, 30)):
    ...
    def list_models(self) -> List[OllamaModel]:
        ...
        if self.is_available():
            try:
                    ...
                    try:
                        ...
                        if info:
                            ...
                            if not model.quantization_level:
                                model.quantization_level = model_info_details.get(
                                    "quantization_level"
                                )
                            if not model.format:
                            ...
                            if modelfile:
                                for line in modelfile.split("\n"):
                                    ...
                                    if (
                                        line_lower.strip().startswith("parameter ")
                                        and "num_ctx" in line_lower
                                    ):
                                        try:
                ...
                logger.debug("Operation failed: %s", e, exc_info=True)
                console.print(
                    f"[yellow]Warning: Could not get models from Ollama API: {e}[/yellow]"
                )
            ...
            for owner_dir in manifests_dir.iterdir():
                ...
                for model_dir in owner_dir.iterdir():
                    ...
                    for tag_file in model_dir.iterdir():
                        ...
                        models.append(
                            OllamaModel(name=model_name, tag=tag, id="", size=size, modified="")
                        )
                ...
                try:
                    ...
                    return (
                        ...
                        (
                            ...
                            f"     sudo cp {path} ~/ollama_models/{safe_name}.gguf\n"
                            f"     sudo chown $USER:$USER "
                            f"~/ollama_models/{safe_name}.gguf\n"
                ...
                try:
                    ...
                    return (
                        ...
                        (
                            ...
                            f"  1. Copy to user directory:\n"
                            f"     mkdir -p ~/ollama_models\n"
                            f"     sudo cp {path} ~/ollama_models/{safe_name2}.gguf\n"
                            f"     sudo chown $USER:$USER "
                            f"~/ollama_models/{safe_name2}.gguf\n"
                            f"  2. Re-pull model (will store in ~/.ollama/models/):\n"
                            f"     ollama pull {name}\n"
                            f"  3. Use ollama directly instead of moxing:\n"
                            f"     ollama run {name}"
...
def print_ollama_models(
    ...
    for m in sorted(models, key=lambda x: x.size, reverse=True):
        ...
        size_str = f"{m.size_gb:.1f}GB" if m.size_gb >= 1 else f"{int(m.size / 1024 ** 2)}MB"
