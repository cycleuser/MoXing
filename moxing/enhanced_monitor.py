...
<body>
    ...
    <script>
        ...
        function initCharts() {
            ...
            memoryChart = new Chart(ctx1, {
                ...
                data: {
                    labels: [],
                    datasets: [
                        { label: 'GPU Memory (MB)', data: [], borderColor: '#00d2ff', backgroundColor: 'rgba(0,210,255,0.1)', fill: true, tension: 0.3 },
                        { label: 'RAM (GB)', data: [], borderColor: '#00ff88', backgroundColor: 'rgba(0,255,136,0.1)', fill: false, tension: 0.3 }
                    ]
                },
            ...
            speedChart = new Chart(ctx2, {
                ...
                data: {
                    labels: [],
                    datasets: [
                        { label: 'Prompt (tok/s)', data: [], borderColor: '#ffd700', fill: false, tension: 0.3 },
                        { label: 'Generate (tok/s)', data: [], borderColor: '#00ff88', fill: false, tension: 0.3 }
                    ]
                },
                options: chartOptions
            });
        ...
        async function updateDisplay() {
            ...
            document.getElementById('generatedTokens').textContent = generatedTokens.toLocaleString();
            ...
            // 计算平均值
            const avgPrompt = historyData.speed.reduce((a, b) => a + b.prompt, 0) / historyData.speed.length;
            const avgGen = historyData.speed.reduce((a, b) => a + b.generate, 0) / historyData.speed.length;
            document.getElementById('avgSpeed').textContent = ((avgPrompt + avgGen) / 2).toFixed(1) + ' tok/s';
            ...
            const avgMemory = historyData.memory.reduce((a, b) => a + b, 0) / historyData.memory.length;
            ...
            const slotsHtml = slots.map(slot => {
                ...
                return `
                    <div class="metric">
                        <span><span class="status-indicator ${statusClass}"></span>Slot ${slot.id}</span>
                        <span class="metric-value">${statusText}</span>
                    </div>
                `;
            }).join('');
            document.getElementById('slotsGrid').innerHTML = slotsHtml || '<div class="metric">No slots</div>';
        ...
        async function init() {
            ...
            if (props.model_path) {
                document.getElementById('modelName').textContent = props.model_path.split('/').pop();
...
def print_enhanced_live_metrics(host: str = "127.0.0.1", port: int = 8080):
    ...
    def generate_display():
        ...
        tokens_table.add_row("Prompt", f"{snapshot.prompt_tokens:,}", f"{snapshot.prompt_speed:.1f} tok/s")
        tokens_table.add_row("Generated", f"{snapshot.generated_tokens:,}", f"{snapshot.generate_speed:.1f} tok/s")
        tokens_table.add_row("Total", f"{snapshot.total_tokens:,}", f"{stats.get('avg_generate_speed', 0):.1f} tok/s")
        ...
        perf_table.add_row("Context", f"{monitor.server_info.context_length:,}" if monitor.server_info.context_length else "N/A")
