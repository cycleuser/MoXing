...
def is_mmproj_tensor(name, config):
    ...
    return ((vision_prefix and name.startswith(vision_prefix)) or
            (audio_prefix and name.startswith(audio_prefix)) or
            (mm_prefix and name.startswith(mm_prefix)))


def split_ollama_gguf(input_path: str, output_dir: str, model_type: str = None):
    """Split Ollama GGUF into language model and mmproj files."""
    ...
    language_path = output_dir / f"{base_name}-language.gguf"
    mmproj_path = output_dir / f"{base_name}-mmproj.gguf"
    
    has_vision = any(t.name.startswith(config['prefixes']['vision']) for t in mmproj_tensors) if config['prefixes']['vision'] else False
    has_audio = any(t.name.startswith(config['prefixes']['audio']) for t in mmproj_tensors) if config['prefixes']['audio'] else False
    ...
    for field in reader.fields.values():
        name = field.name
        if not ('vision' in name.lower() or 'audio' in name.lower() or 
                name.startswith('clip.') or name.startswith('mmproj.')):
    ...
    if has_vision:
        ...
        writer.add_key_value('clip.vision.image_size', 0, GGUFValueType.UINT32)  # 0 for dynamic image size
        ...
        for orig_key, mapped_key in key_mapping.items():
            if orig_key in vision_metadata:
                ...
                value_type = GGUFValueType.UINT32 if isinstance(value, int) else GGUFValueType.FLOAT32
        ...
        if 'projector.scale_factor' in vision_metadata:
            writer.add_key_value('clip.vision.projector.scale_factor', 
                                 vision_metadata['projector.scale_factor'], GGUFValueType.UINT32)
...
def main():
    parser = argparse.ArgumentParser(description="Split Ollama GGUF into language model and mmproj files")
    parser.add_argument("input", help="Input Ollama GGUF file path")
    parser.add_argument("-o", "--output-dir", default="./", help="Output directory")
    parser.add_argument("-t", "--model-type", choices=MODEL_CONFIGS.keys(), help="Model type (auto-detected if not specified)")
