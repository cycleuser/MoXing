...
def extract_mmproj(input_path: str, output_path: str, model_type: Optional[str] = None):
    ...
    has_vision = (
        any(t.name.startswith(vision_prefix) for t in mmproj_tensors) if vision_prefix else False
    )
    has_audio = (
        any(t.name.startswith(audio_prefix) for t in mmproj_tensors) if audio_prefix else False
    )
    ...
    if has_vision:
        if "attention.head_count" in vision_params:
            writer.add_key_value(
                Keys.ClipVision.Attention.HEAD_COUNT,
                vision_params["attention.head_count"],
                GGUFValueType.UINT32,
            )
        if "block_count" in vision_params:
            writer.add_key_value(
                Keys.ClipVision.BLOCK_COUNT, vision_params["block_count"], GGUFValueType.UINT32
            )
        if "embedding_length" in vision_params:
            writer.add_key_value(
                Keys.ClipVision.EMBEDDING_LENGTH,
                vision_params["embedding_length"],
                GGUFValueType.UINT32,
            )
            writer.add_key_value(
                Keys.ClipVision.PROJECTION_DIM,
                vision_params["embedding_length"],
                GGUFValueType.UINT32,
            )
        if "feed_forward_length" in vision_params:
            writer.add_key_value(
                Keys.ClipVision.FEED_FORWARD_LENGTH,
                vision_params["feed_forward_length"],
                GGUFValueType.UINT32,
            )
        if "patch_size" in vision_params:
            writer.add_key_value(
                Keys.ClipVision.PATCH_SIZE, vision_params["patch_size"], GGUFValueType.UINT32
            )

        writer.add_key_value(Keys.ClipVision.IMAGE_SIZE, 224, GGUFValueType.UINT32)
        writer.add_key_value(
            Keys.ClipVision.IMAGE_MEAN,
            config["image_mean"],
            GGUFValueType.ARRAY,
            GGUFValueType.FLOAT32,
        )
        writer.add_key_value(
            Keys.ClipVision.IMAGE_STD,
            config["image_std"],
            GGUFValueType.ARRAY,
            GGUFValueType.FLOAT32,
        )
