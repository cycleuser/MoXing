...
class BenchmarkRunner:
    ...
    BENCHMARK_PROMPTS = {
        ...
        "standard": (
            ...
            "including the concepts of weights, biases, activation functions, "
            "and backpropagation."
        ),
        "code": (
    ...
    def run(
        ...
        **kwargs,
        ...
        prompt_text = self.BENCHMARK_PROMPTS.get(prompt, prompt)
        ...
        for run_idx in range(n_runs + (1 if warmup else 0)):
            ...
            if is_warmup:
                ...
                run_num = run_idx + 1 - (1 if warmup else 0)
                console.print(
                    f"[blue]Running benchmark {run_num}/{n_runs}...[/blue]"
                )
    ...
    def compare_models(
        self, models: List[str], prompt: str = "standard", n_tokens: int = 128, **kwargs
    ...
    def print_results(self, result: BenchmarkResult):
        ...
        console.print(
            Panel(
                ...
                f"[green]GPU Layers:[/green] "
                f"{'all' if result.gpu_layers < 0 else result.gpu_layers}",
...
def benchmark_model(
    model: str, prompt: str = "standard", n_tokens: int = 128, **kwargs
) -> BenchmarkResult:
...
def estimate_speed(
    model_size_gb: float, gpu_memory_gb: float, backend: str = "vulkan"
) -> Dict[str, Any]:
    """
