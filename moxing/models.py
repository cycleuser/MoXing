...
DEFAULT_MODEL_DIR = Path.home() / ".cache" / "moxing" / "models"
...
class ModelDownloader:
    ...
    def search(self, query: str, source: str = "auto", limit: int = 20) -> List[ModelInfo]:
    ...
    def _search_hf(self, query: str, limit: int) -> List[ModelInfo]:
        ...
        try:
            resp = self._client.get(
                ...
                params={"search": f"{query} gguf", "limit": limit, "filter": "gguf"},
            )
            ...
            logger.debug("GGUF file listing failed: %s", e, exc_info=True)
            ...
            for search_query in [f"{query}-GGUF", f"{query}-gguf", query]:
                ...
                try:
                    models = api.list_models(
                        owner_or_group=search_query, page_size=min(limit * 3, 50)
                    )
                    if not models:
                        continue
                    for item in models:
                        ...
                        if not repo_id and name:
                            ...
                            repo_id = f"{org}/{name}" if org else name
    ...
    def _search_modelscope_api(self, query: str, limit: int) -> List[ModelInfo]:
        ...
        try:
            resp = self._client.get(
                f"{self.MS_API}/models", params={"Name": query, "PageSize": limit}
            )
            resp.raise_for_status()
    ...
    def _list_gguf_files_modelscope_lib(
        self, api, repo_id: str, model_info: Optional[dict] = None
    ) -> List[tuple]:
    ...
    def _list_gguf_files_modelscope(self, repo_id: str) -> List[tuple]:
        ...
        files = []
        try:
            resp = self._client.get(f"{self.MS_API}/models/{repo_id}/repo/files", timeout=30)
            resp.raise_for_status()
    ...
    def download(
        ...
        console.print(
            f"[blue]Downloading {filename} ({size / (1024**3):.2f} GB) from {source}...[/blue]"
        )
    ...
    def download_with_hf_hub(
        self, repo: str, filename: Optional[str] = None, token: Optional[str] = None
    ) -> Path:
    ...
    def download_with_modelscope(self, repo: str, filename: Optional[str] = None) -> Path:
        """Download using modelscope library."""
        try:
            from modelscope import snapshot_download
        except ImportError as e:
            raise ImportError("modelscope not installed. Run: pip install modelscope") from e
        ...
        local_dir = snapshot_download(model_id=repo, allow_patterns=[filename])
