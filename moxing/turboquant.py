...
class BetaDistribution:
    ...
    def pdf(self, x: float) -> float:
        ...
        if self._normal_approx:
            return (1.0 / (self.scale * math.sqrt(2 * math.pi))) * math.exp(
                -0.5 * (x / self.scale) ** 2
            )
...
class LloydMaxQuantizer:
    ...
    def get_codebook(cls, dim: int, bits: int) -> np.ndarray:
        ...
        if bits in cls.OPTIMAL_CENTROIDS:
            ...
            codebook_list: Any = []
            for c in centroids:
                codebook_list.extend([-c * scale, c * scale])
            codebook = np.array(sorted(codebook_list), dtype=np.float32)
        else:
    ...
    @classmethod
    def _compute_codebook_lloyd_max(cls, dim: int, bits: int, n_iter: int = 100) -> np.ndarray:
    ...
    def quantize(cls, x: np.ndarray, codebook: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        x_flat = x.flatten()
        indices: np.ndarray = np.zeros(len(x_flat), dtype=np.int32)
...
class QJLQuantizer:
    ...
    def quantize(self, x: np.ndarray) -> np.ndarray:
        ...
        projected = self.S @ x if x.ndim == 1 else x @ self.S.T
    ...
    def inner_product(
        self, z1: np.ndarray, z2: np.ndarray, scale1: float = 1.0, scale2: float = 1.0
    ) -> float:
        """Estimate inner product from quantized vectors."""
        if z1.ndim == 1:
            hamming: Any = np.sum(z1 != z2)
...
class TurboQuant:
    ...
    def quantize(self, x: np.ndarray, norms: Optional[np.ndarray] = None) -> Dict[str, Any]:
        """
        ...
        x_rotated = self._rotate(x_normalized) if self.config.use_rotation else x_normalized.copy()
    ...
    def _quantize_mse(self, x_rotated: np.ndarray, norms: np.ndarray) -> Dict[str, Any]:
        """MSE-optimal quantization."""
        if self.codebook_mse is None:
            raise ValueError("Codebook not initialized - call fit() first")
        indices, reconstructed = LloydMaxQuantizer.quantize(x_rotated, self.codebook_mse)
    ...
    def _quantize_inner_product(self, x_rotated: np.ndarray, norms: np.ndarray) -> Dict[str, Any]:
        ...
        for i in range(n_vectors):
            ...
            if residual_norm > 1e-8 and self.qjl is not None:
    ...
    def _dequantize_inner_product(self, compressed: Dict[str, Any]) -> np.ndarray:
        ...
        for i in range(n_vectors):
            if self.codebook_mse is None:
                raise ValueError("Codebook not initialized - call fit() first")
            reconstructed[i] = LloydMaxQuantizer.dequantize(indices[i], self.codebook_mse)

            if residual_norms[i] > 1e-8 and self.qjl is not None:
...
class TurboQuantMixedPrecision:
    ...
    def _setup_quantizers(self):
        ...
        self.outlier_quantizer = TurboQuant(
            TurboQuantConfig(
                dim=self.n_outliers,
        ...
        self.normal_quantizer = TurboQuant(
            TurboQuantConfig(
    ...
    def identify_outliers(self, x: np.ndarray) -> np.ndarray:
        ...
        threshold: Any = np.percentile(channel_vars, 100 * (1 - self.n_outliers / self.dim))
    ...
    def dequantize(self, compressed: Dict[str, Any]) -> np.ndarray:
        ...
        outlier_reconstructed = self.outlier_quantizer.dequantize(compressed["outlier_compressed"])
        normal_reconstructed = self.normal_quantizer.dequantize(compressed["normal_compressed"])
...
def benchmark_turboquant_comprehensive():
    ...
    for name, bits in configs:
        ...
        print(
            f"{name:<20} {sizes[0]:<10.2f} {sizes[2]:<10.2f} "
            f"{sizes[4]:<10.2f} {compression:<12.1f}x {quality}"
        )
