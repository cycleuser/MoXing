...
class GGUFParser:
    ...
    OLLAMA_ONLY_ARCHITECTURES = {"glm4moelite", "glm4moe", "bitnet"}
    ...
    def _build_metadata(self, version: int) -> GGUFMetadata:
        ...
        if version > GGUF_VERSION:
            warnings.append(
                f"GGUF version {version} is newer than supported"
                f" ({GGUF_VERSION}). Update llama.cpp."
            )
...
def check_compatibility(path: Path) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        logger.debug("GGUF compatibility check failed: %s", e, exc_info=True)
    ...
    try:
        ...
        if not meta.is_valid:
            ...
            if meta.architecture in ("gemma3", "qwen3", "qwen35"):
                suggestions.append(
                    f"This is a {meta.architecture.upper()} model which requires "
                    f"the latest llama.cpp or MLX"
                )
