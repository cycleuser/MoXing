...
from moxing.device import BackendType, Device, DeviceConfig, DeviceDetector, estimate_model_size_gb
from moxing.models import ModelDownloader, ModelInfo, ModelRegistry
from moxing.runners.base import BaseRunner, RunnerConfig, create_runner, detect_best_runner
...
class AutoRunner:
    ...
    def detect_config(self, model_path: Union[str, Path], ctx_size: int = 4096) -> RunnerConfig:
        ...
        device_config = None
        if self.auto_detect_device and self._detector is not None:
    ...
    def download(self, model: str, quant: str = "Q4_K_M", source: str = "auto") -> Path:
    ...
    def run(
        ...
        with runner:
            ...
            if chat:
                ...
                resp = client.post("/completion", json={"prompt": prompt, "n_predict": n_tokens})
                return resp.json().get("content", "")

    def resolve_model(self, model: str, quant: str = "Q4_K_M", source: str = "auto") -> Path:
        ...
        for local_model in self._downloader.get_local_models():
            if (
                model.lower() in local_model.name.lower()
                or model.lower() in local_model.repo.lower()
            ) and (
                quant.lower() in local_model.quantization.lower() or not quant
            ) and local_model.local_path is not None:
    ...
    def quick_chat(self, model: str, prompt: str, quant: str = "Q4_K_M", **kwargs) -> str:
...
def quick_run(
    model: str, prompt: str = "Hello!", quant: str = "Q4_K_M", runner_type: str = "auto", **kwargs
) -> str:
...
def quick_server(
    model: str, quant: str = "Q4_K_M", port: int = 8080, runner_type: str = "auto", **kwargs
) -> BaseRunner:
