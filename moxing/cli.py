...
def serve(
    ...
    ngl: int = typer.Option(-1, "--ngl", "--gpu-layers", help="Number of GPU layers (-1=auto, 0=CPU only, 999=all GPU)"),
        ...
        if effective_runner in ("vllm",):
            ...
            try:
                server = LlamaServer(
                    ...
                    port=port,
                    ctx_size=ctx_size,
                    n_gpu_layers=ngl if ngl >= 0 else device_config.n_gpu_layers,
                    device=device_str,
